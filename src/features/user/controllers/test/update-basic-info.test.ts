...
import { authUserPayload, authMockReq, authMoqRes } from '@root/mocks/auth.mock';
...
describe('EditBasicInfo', () => {
  ...
  describe('info', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      const req: Request = authMockReq({}, basicInfo, authUserPayload, {}) as Request;
    ...
    it('should call updateUserInfoInDB', async () => {
      const basicInfo = {
        quote: 'This is cool',
        work: 'KickChat Inc.',
        school: 'Taltech',
  ...
  describe('social', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      const req: Request = authMockReq({}, socialInfo, authUserPayload, {}) as Request;
      const res: Response = authMoqRes();
      jest.spyOn(UserCache.prototype, 'updateSingleUserItemInCache');
    ...
    it('should call updateUserLinksInDB', async () => {
      const socialInfo = {
        facebook: 'https://facebook.com/tester',
        instagram: 'https://instagram.com',
        youtube: 'https://youtube.com',
      ...
      await Edit.prototype.social(req, res);
      expect(userQueue.addUserJob).toHaveBeenCalledWith('updateUserLinksInDB', {
        key: `${req.currentUser?.userId}`,
        value: req.body
      });
      expect(res.status).toHaveBeenCalledWith(200);
