...
class PostService {
  ...
  public async postsCount(): Promise<number> {
    const total: number = await PostModel.find({}).countDocuments();
  ...
  public async deletePost(postId: string, userId: string): Promise<void> {
    ...
    const updateUserPostsCount: UpdateQuery<IUserDocument> = UserModel.updateOne({ _id: userId }, { $inc: { postsCount: -1 } });
  ...
  public async editPost(postId: string, updatedPost: IPostDocument): Promise<void> {
    const editPost: UpdateQuery<IUserDocument> = PostModel.updateOne({ _id: postId }, { $set: updatedPost });
    await Promise.all([editPost]);
  }
}
