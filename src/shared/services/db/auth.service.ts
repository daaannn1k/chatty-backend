...
class AuthService {
  ...
  public async getUserByUsernameOrEmail(username: string, email: string): Promise<IAuthDocument | null> {
    ...
    const user: IAuthDocument | null = (await AuthModel.findOne(query).exec()) as IAuthDocument | null;
    return user;
  }
  ...
  public async getAuthUserByEmail(email: string): Promise<IAuthDocument | null> {
    const user: IAuthDocument | null = (await AuthModel.findOne({ email: Helpers.lowerCase(email) }).exec()) as IAuthDocument | null;
    return user;
  }
