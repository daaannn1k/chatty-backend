...
export class PostCache extends BaseCache {
  ...
  public async savePostToCache(data: ISavePostToCache): Promise<void> {
    ...
    const { _id, userId, username, email, avatarColor, profilePicture, post, bgColor, feelings, privacy, gifUrl, commentsCount, imgVersion, imgId, reactions, createdAt} = createdPost;
  ...
  public async getTotalPostsInCache(): Promise<number> {
    try {
      ...
      const totalPosts: number = await this.client.ZCARD('post');
      return totalPosts;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again');
  ...
  public async getPostsByUserFromCache(key: string, uId: number): Promise<IPostDocument[]> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }
  ...
  public async getByUserTotalPostsInCache(uId: number): Promise<number> {
    try {
      if (!this.client.isOpen) {
        await this.client.connect();
      }

      const totalPosts: number = await this.client.ZCOUNT('post', uId, uId);
      return totalPosts;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again');
  ...
  public async updatePostInCache(key: string, updatedPost: IPostDocument): Promise<IPostDocument> {
    ...
    try {
      ...
      const post: IPostDocument[] = reply as IPostDocument[];
